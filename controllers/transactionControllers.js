...
const getTransactions = async (req, res) => {
  try {
    ...
    if (schoolFilter) {
      matchStage["school_id"] = {
        $in: schoolFilter.map(id => new mongoose.Types.ObjectId(id))
      };
    }

    if (search) {
      matchStage["order_status.custom_order_id"] = { $regex: search, $options: "i" };
    }
    if (startDate && endDate) {
      ...
      matchStage["order_status.payment_time"] = { $gte: start, $lte: end };
    }
